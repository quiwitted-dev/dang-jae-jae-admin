...
const ProjectCard = ({ item, isFavorite }: ProjectCardProps) => {
  ...
  return (
    <Card className="relative flex flex-col overflow-hidden bg-transparent p-0 rounded-4xl aspect-300/220 min-w-[350px] max-w-[500px] mx-auto justify-between">
      <Link href={`/${item.id}?source=${item.dataSource}`}>
        <Image
